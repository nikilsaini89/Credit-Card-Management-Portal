...
<template>
  <div class="page-root">
    <!-- Debug banner -->
    <div style="background: #ff0; padding: 6px; font-weight: bold; margin-bottom:10px;">
      DEBUG: CardDetail mounted — Route ID = {{ $route.params.id }}
    </div>
          ...
          <div class="panel overview">
            ...
            <div class="overview-grid">
              <div class="ov-left">
                <div class="row">
                  <div class="rlabel">Credit Limit</div>
                  <div class="rval">₹{{ formatINR(totalLimit) }}</div>
                </div>
                <div class="row">
                  <div class="rlabel">Outstanding Balance</div>
                  <div class="rval danger">₹{{ formatINR(outstanding) }}</div>
                </div>
                <div class="row">
                  <div class="rlabel">Available Credit</div>
                  <div class="rval success">₹{{ formatINR(availableCredit) }}</div>
                </div>
                <div class="util-row">
                  <div class="rlabel">Credit Utilization</div>
            ...
            <ul class="tx-list">
              <li v-for="tx in recentTransactionsList" :key="tx.id" class="tx">
                <div class="tx-left">
                  ...
                  <div v-if="tx.mode" class="tx-processor">{{ tx.mode }}</div>
                </div>
                <div class="tx-right">
                  <div :class="['tx-amount', (Number(tx.amount) < 0) ? 'neg' : 'pos']">
                    {{ formatCurrency(tx.amount) }}
...
console.log("🟢 File loaded: CardDetail.vue")
...
export default {
  ...
  setup() {
    ...
    const route = useRoute()
    console.log("🟢 route params at setup:", route.params)
    ...
    const mergedCard = computed(() => {
      if (!card.value) return null
      return { ...card.value, fullPan: fullPan.value || null, cvv: card.value.cvv ?? null }
    })
    const recentTransactionsList = computed(() => {
      return (transactions.value || []).map(tx => ({
        ...tx,
    ...
    function handleCardAction({ action }) {
      console.log("🟡 handleCardAction called:", action)
      if (action?.to) router.push(action.to)
    }
    function handleCardBlock(payload) {
      console.log("🟡 handleCardBlock called:", payload)
      if (payload?.card) payload.card.status = payload.newStatus
    }
    function openLimitModal() {
      console.log("🟡 openLimitModal clicked")
      if (card.value) isLimitModalOpen.value = true
    }

    async function handleLimitNext(payload) {
      console.log("🟡 handleLimitNext called:", payload)
      const selectedNewLimit = Number(payload?.newLimit || 0)
      if (!selectedNewLimit) return
      pendingLimit.value = selectedNewLimit
      isLimitModalOpen.value = false
    ...
    async function loadCard() {
      ...
      try {
        ...
        const cardsList = await getCards()
        console.log("🔵 getCards() response:", cardsList)

        const matched = (cardsList || []).find(c => String(c.id) === String(id))
        console.log("🔵 matched card:", matched)
        ...
        const dash = await getDashboard()
        console.log("🔵 getDashboard() response:", dash)
    ...
    return {
      card, transactions, userName, loading, error,
      isLimitModalOpen, pendingLimit, updating,
      cardLastFour, utilizationPercentage, recentTransactionsList,
      formatINR, formatCurrency, formatDateTime,
      cardActionsMenu, handleCardAction, handleCardBlock,
      openLimitModal, handleLimitNext, fullPan, mergedCard,
      totalLimit, availableCredit, outstanding, loadCard
    }
  }
}
</script>
