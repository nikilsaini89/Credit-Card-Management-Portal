...
public class TransactionService {
    ...
    public TransactionResponse createTransaction(CreateTransactionRequest request) {
        ...
        CreditCardEntity card = creditCardRepository.findById(request.getCardId())
                .orElseThrow(() -> new CreditCardNotFoundException("Card not found with ID: " + request.getCardId()));
    ...
    public TransactionHistoryResponse getTransactionHistory(Long cardId, int page, int size, 
        ...
        if (category != null || isBnpl != null || merchantName != null) {
            ...
            transactionPage = transactionRepository.findByCardIdOrderByTransactionDateDesc(cardId, pageable);
        }
    ...
    public AnalyticsResponse getSpendingAnalytics(Long cardId) {
        ...
        Long bnplCount = (long) transactionRepository.findByCardIdAndIsBnplTrueOrderByTransactionDateDesc(cardId).size();
        ...
        List<AnalyticsResponse.CategorySpending> categoryBreakdown = categoryData.stream()
                .map(data -> {
                    ...
                    Double percentage = totalSpent > 0 ? (amount.doubleValue() / totalSpent) * 100 : 0.0;
        ...
        BigDecimal averageAmount = transactionCount > 0 ? 
                BigDecimal.valueOf(totalSpent).divide(BigDecimal.valueOf(transactionCount), 2, BigDecimal.ROUND_HALF_UP) : 
                BigDecimal.ZERO;
