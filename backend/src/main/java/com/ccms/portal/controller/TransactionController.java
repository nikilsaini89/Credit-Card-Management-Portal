...
public class TransactionController {
    ...
    public ResponseEntity<List<Object[]>> getMonthlySpendingTrend(
            @PathVariable Long cardId,
            @RequestParam(defaultValue = "#{T(java.time.LocalDate).now().minusMonths(12)}") LocalDate startDate) {
        
        log.info("Received request for monthly spending trend - cardId: {}, startDate: {}", cardId, startDate);
        
        try {
            List<Object[]> response = transactionService.getSpendingAnalytics(cardId).getMonthlyTrends()
                    .stream()
                    .map(trend -> new Object[]{trend.getYear(), trend.getMonth(), trend.getAmount()})
                    .collect(java.util.stream.Collectors.toList());
    ...
    public ResponseEntity<Map<String, Object>> getTotalSpent(@PathVariable Long cardId) {
        ...
        try {
            ...
            log.info("Retrieved total spent for card: {} - Amount: {}, Count: {}", cardId, totalSpent, transactionCount);
            return ResponseEntity.ok(response);
        } catch (Exception e) {
